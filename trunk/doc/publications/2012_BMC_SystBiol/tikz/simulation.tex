...
\begin{tikzpicture}[node distance = 2cm, auto]
	...
	\node [block, below of=start] (init) {$t:=t_{begin}$\qquad $x:=x_{begin}$ \quad $s:=s_0$};
	\node [io, below of=init] (output) {($x_{res}$, $t_{res}$):=Rosenbrock(t, x, s)};
	\node [block, below of=output,text width=17em] (pevents) {$x_p$:=ProcessEventsAndRules($x_{res}, t_{res}$)};
	\node [decision, below of=pevents, node distance=3cm] (xpxres?) {$x_p=x_{res}?$};
	\node [block, left of=xpxres?, below of=xpxres?, node distance=4cm] (newvalues) {$t:=t_{res}$ \qquad $x:=x_{res}$};
...
%	\node [cloud, below of=rulesleft?, node distance=2.5cm] (stop) {Stop};
	...
	\path [line] (end?) -| (aux) |- node {No}(init);
