...
The described algorithm is also displayed in Figure 1.
...
For given values $\vec{Q}$ at a time point the curring vector of derivatives is calculated as follows: First all derivatives in the vector $\vec{Q}'$ are set to zero. Then the rate rules are processed, which can change $\vec{Q}'$. After that for each reaction the velocity, which depends on $\vec{Q}$, is computed with the help of the syntax graph. Then the derivatives of all species that participate in the current reaction need to be updated.

\subsection{Integrated calculation for a certain time step including event processing}
The Rosenbrock solver is the solver that can adapt its step size if events are happening. For a certain time begin and time end and a vector $\vec{Q}$ it determines a new vector at a time point that is determined by the current adaptive step size. It refers to the already described calculation of the derivatives for a certain time point in this step.
...
\section*{Figures}
 \subsection*{Figure 1 - Algorithm for transforming algebraic rules to assignment rules}
...
The first step is to decide whether the model is overdetermined by creating a matching of the variables. If this is not the case, every algebraic rule is solved to the matched variable, which provides the basis for the creation of an equivalent assignment rule.
 ...
 \subsection*{Figure 2 - Processing of events: algorithm}

The list of the events running and the already processed events are saved. In each step the trigger of every event that is not contained in any of the two lists is evaluated. If it is true, it is added to the running events. After the procession of all triggers one event running is chosen, which can be done randomly. This event is then processed and afterwards added to the list of processed events.
