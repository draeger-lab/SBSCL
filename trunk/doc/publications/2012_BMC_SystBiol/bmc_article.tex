...
such as the \acf{SBML} \cite{Hucka2004, SBML2013} \COR{and} CellML \cite{Lloyd2004, CellML2013}
enable encoding of quantitative biological network models.
To facilitate sharing and \COR{re-use} of the models, online \COR{databases} such as
...
\COR{When}\sout{In case that} the system under study operates at multiple time scales, i.e., it
...
with the help of the unified syntax graph (\COR{figure~1} shows an example of
such a graph).
...
\COR{BioModels Database contains a collection of published and curated models.
An implementation of an \acs{SBML} solver that passes the test suite should
in principle also be capable of computing the solution of all models from
BioModels Database.
This database currently provides neither reference data for the models,
nor any settings for the numerical computation (such as step size, end time etc.).
...
\COR{The modifications done to the Rosenbrock solver enables a precise timing of events during simulation. 
Albeit when events are triggered in small intervals, this can lead to noticeable increase in run-time. This can
for example be observed in model \numero~408 of the BioModels Database. When the precise timing of events is not of utmost importance, a different solver other than the Rosenbrock solver can be chosen. But there are ambitions to improve the runtime behavior during the precise timing of events.}

\COR{When dealing with stiff problems, Rosenbrock's method is a good choice, since it is intended for stiff
\acsp{ODE}. However the experiments show, that the Rosenbrock solver can be inefficient for non-stiff
 problems in comparison to other solvers. Whereas more advanced solvers, such as \textsc{CVODE} from
 \textsc{SUNDIALS} \cite{Hindmarsh2005}CVODE, can adapt for non-stiff and stiff problems, the Rosenbrock
 solver can not. This issue can lead to an increase in run-time regarding large models like model
 \numero~235 of the BioModels Database. In some cases, tuning the relative and absolute tolerance can
 help, but depending on the system's structure, Rosenbrock's method is sometimes stretched to its limits. 
%In order to maintain the desired accuracy, the solver continuously decreases its step size.
The Runge-Kutta-Fehlberg method \cite{Fehlberg1970} (\acs{KiSAO} term 86), which is included in iBioSim, shows also an increase in run time concerning the model mentioned above.
...
\COR{The performance of the methods Runge-Kutta-Fehlberg and Rosenbrock show, however, that some
 \acs{ODE} solvers can have more difficulties with some biological models than more advanced solvers, such
 as \textsc{CVODE}, which is incorporated into BioUML. \textsc{SUNDIALS} can handle complicated \acsp{ODE} significantly better, but since it is not available under the \acs{LGPL} and currently no open-source Java version of these solvers can be obtained, we disregard its use for now. }

\COR{Nevertheless, the implemented Rosenbrock solver embodies a universal approach for simulation, that can deal with
stiff problems and precisely solve models containing arbitrary \acs{SBML} elements.}
...
\COR{with appropriate settings (see Methods)}.
...
However, when such an event is cancelled by other events, also all of its assignments are cancelled before execution.
...
as stated in Results, the execution of one event can influence the priority \COR{of} the remaining events.
\COR{Consider the concept binary max heap: it can possibly happen that many priorities simultaneously change,
...
The Rosenbrock solver with its adaptive step size is the most effective solver in this library for stiff \acsp{ODE}.
...
\COR{All curated models from BioModels Database have been simulated with identical settings, as it was suggested before \cite{Bergmann2008}}:
time interval $[0, 10]$, the Rosenbrock solver, $10^{-6}$ as relative and $10^{-12}$ as absolute tolerance, and a step size of $0.01$ time units.
