...
the entire SBML Test Suite \CORR{version~2.3.2} and %also been used to simulate 
all models of \href{http://www.ebi.ac.uk/biomodels-main/}{BioModels Database} \CORR{(release~23, October 2012)}.
...
An assignment rule also influences the absolute value of \sout{some}\COR{a subject} quantity.
...
\sout{For}\COR{At} every time step, the events to be executed are a union of two
subsets of the set of all events.
On \sout{the} one hand, \COR{there are events whose triggers have} been
...
For every element of the resulting set of events\sout{ their}\COR{, the}
priority rule must be evaluated.
One event is randomly chosen for execution from all events \COR{of} highest
priority.
\COR{In principle, a}ll other events could be \sout{handled}\COR{processed} in
...
In this case, \sout{another}\COR{the} event that has now the highest priority is
chosen \COR{as next}.
...
file to \sout{this}\COR{the} interpreter \COR{in this library}.
...
\sout{the}\COR{its} \acs{SBML} interpreter against the entire SBML Test Suite
...
SBML Test Suite \CORR{version~2.3.2} \COR{\cite{SBMLtestSuite2013}} using the Rosenbrock solver.
...
\COR{Two events fire every $10^{-2}$\,s \CORR{for 1.000 seconds}.
...
\href{http://www.ebi.ac.uk/biomodels-main/}{BioModels Database} 
\CORR{(release~23, October 2012)}, a resource that contains a
...
\cite{Takizawa2013}, and VCell \cite{Moraru2008}\CORR{, \cite{Resasco2012}}.
...
There are already ambitions to improve the runtime behavior but still achieve a
...
Albeit there are some models from
\href{http://www.ebi.ac.uk/biomodels-main/}{BioModels Database} that constitute
very stiff problems or large \acsp{ODE} (e.g., the models \numero~235
\citep{Kuehn2009} or \numero~408 \citep{Hettling2011}),
...
\sout{Please n}\COR{N}ote that in the \COR{SBML Test Suite column, a dash means
...
\cite{Madsen2012}.} \CORR{Furthermore, some programs, e.g. VCell, do not use SBML as their native format.}
...
Th\COR{is}\sout{e} table shows the number of tested models and the total running
