...
As a result, \TODO{99.06\,\%} of these models could be \TODO{correctly} simulated.
...
programs.
\COR{It should be mentioned that this comparsion can only mirror a snapshot of
...
\COR{As mentioned before, Rosenbrock's solver is basically capable of solving stiff \acsp{ODE}.
Albeit there are some models from BioModels Database that constitute very stiff problems and large \acsp{ODE}
(e.g., the models \numero~235 or \numero~408), for which Rosenbrock's method has some difficulty solving them.
In order to maintain the desired accuracy, the solver contineously decreases its step size.
...
\COR{This behavior can also be observed for the Runge-Kutta-Fehlberg method \cite{Fehlberg1970} (\acs{KiSAO} term 86), which is included in iBioSim.
The focus of iBioSim, however, lies not in the use of \acs{ODE} solvers, but in the stochastic analysis of systems with the Reb2Sac stochastic simulator and abstraction techniques to make the models easier to analyze.}

\COR{The performance of the Runge-Kutta-Fehlberg and Rosenbrock show that some \acs{ODE} solvers can have more difficulties with some biological models than
more advanced solvers, such as CVODE from SUNDIALS \cite{Hindmarsh2005}, which is incorporated into BioUML. 
SUNDIALS can handle complicated \acsp{ODE} significantly better, but since it is not available under the \acs{LGPL} and currently no open-soruce Java version of these solvers can be obtained, we disregard its use.
...
SBML Test Suite (version~2.0.2) \COR{\cite{SBMLtestSuite2013}} using Rosenbrock's solver.
...
\COR{Generally, event handling is the most time consuming step during the simulation of an \acs{SBML} model.
In this particular model, two of the 23 events fire every $10^{-2}$\,s.
...
\cite{Novere2006a})
\COR{have been simulated with identical seetings
(time interval $[0, 10]$, Rosenbrock's solver, and a step size of $0.1$ time units)}.
The Systems Biology Simulation Core Library \COR{is able to solve} \TODO{99.06\,\%} of \COR{all these models}. 
These results suggest the reliability of the simulation algorithm described in
this work.

Our tests indicate that \COR{at the moment} \TODO{only two programs} pass the entire \COR{test suite} for all 
...
This is why efficient and clearly organized data structures are required to ensure
\COR{high performance of the library}.
\TODO{DISCUSSION-END!!!}
...
and the exceution of its first assignment.}
...
ques like this would apply a well known binary max heap data structure.
...
%
\COR{Besides the subset of MathML functions provided by the different \acs{SBML} specifications there exists also the possibility to apply user-defined functions from a given \acs{SBML} file.
...
The evaluation of a MathML element with a user-defined function take place in several steps.
...
At last the complete MathML element can be evaluated.}
...
Since SBML Level~3 Version~1, the stoichiometry of a reaction can be directly altered because it is now possible to address the identifier of a \SpeciesReference as the target variable within an \AssignmentRule.
...
solver with an absolute tolerance of \TODO{xxx} and a relative tolerance of \TODO{xxx}.
...
The user has the possibility to specify the end of the simulation.
