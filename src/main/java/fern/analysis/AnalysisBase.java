 ...
 * AutocatalticNetworkDetection, ShortestPath, ...). It creates adjacency lists for the molecule
 * species (which are usually not present in {@link Network} implementing classes) and presents
 * methods for a general breath first search and depth first search, which can be controlled by a
 * {@link NetworkSearchAction}
 *
...
public class AnalysisBase {
  ...
  /**
   * Contains the adjacency list for molecule species towards reactions, where the species is a
   * reactant.
   */
  ...
  /**
   * Creates an analysis instance. In order to do that, a {@link Network} is required. If the
   * network is a {@link ModifierNetwork}, the original network is also discovered and stored.
   *
  ...
  public AnalysisBase(Network network) {
    ...
    originalNetwork =
        network instanceof ModifierNetwork ? ((ModifierNetwork) network).getOriginalNetwork()
            : network;
  }

  /**
   * Creates the adjacency lists for the molecule species. A subclass has to invoke this before it
   * can use the protected fields adjListAsRea and adjListAsPro.
   */
   ...
   * <code>speciesSource</code> and <code>reactionSource</code> are the initial content of the
   * queue. The search is controlled by an {@link NetworkSearchAction}.
   *
   * @param speciesSource  indices of the species to start with
   * @param reactionSource indices of the reactions to start with
   * @param action         controls what species/reactions have to be visited and what to do after
   *                       discovering/finishing a species/reaction
   ...
   * <code>speciesSource</code> and <code>reactionSource</code> are the initial content of the
   * stack. The search is controlled by an {@link NetworkSearchAction}.
   *
   * @param speciesSource  indices of the species to start with
   * @param reactionSource indices of the reactions to start with
   * @param action         controls what species/reactions have to be visited and what to do after
   ...
   * <code>speciesSource</code> and <code>reactionSource</code> are the initial content of the
   * search structure {@link IntSearchStructure}. The search is controlled by an {@link
   * NetworkSearchAction}.
   *
   ...
   */
  public int search(IntSearchStructure str, int[] speciesSource, int[] reactionSource,
      NetworkSearchAction action) {
    if (adjListAsPro == null) {
      ...
      if (queueElement != index) { // reaction
        ...
        if (additionals != null) {
          for (int n : additionals) {
            if (!speciesDiscovered.get(n) && action
                .checkSpecies(n, NeighborType.Additional)) {
              speciesDiscovered.set(n);
              str.add(n);
              action.speciesDiscovered(n);
            }
        ...
        if (additionals != null) {
          for (int n : additionals) {
            if (!reactionDiscovered.get(n) && action
                .checkReaction(n, NeighborType.Additional)) {
              reactionDiscovered.set(n);
              str.add(encode(n));
              action.reactionDiscovered(n);
            }
