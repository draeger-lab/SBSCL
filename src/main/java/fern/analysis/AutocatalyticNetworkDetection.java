...
public class AutocatalyticNetworkDetection extends AnalysisBase {
	 ...
	 * The network has to implement <code>CatalystIterator</code>, otherwise an <code>IllegalArgumentException</code> is thrown.
	 * 
...
//			NetworkTools.dumpNetwork(new ExtractSubNetwork(network,getAutocatalyticReactions(),getAutocatalyticSpecies()));
	...
	/**
	 * Gets the autocatalytic species as {@link BitVector}. Throws a <code>RuntimeException</code>
	 * if the detection algorithms has not been called.
	 * 
	...
	/**
	 * Returns true if the given reaction is autocatalytic. Throws a <code>RuntimeException</code>
	 * if the detection algorithms has not been called.
	 * 
	...
	/**
	 * Returns true if the given species is autocatalytic. Throws a <code>RuntimeException</code>
	 * if the detection algorithms has not been called.
	 * 
	...
	protected int[] getFoodSpecies() {
		...
		for (int i=0; i<network.getNumSpecies(); i++)
			if (network.getSpeciesName(i).length()==1) re.add(i);
		return NumberTools.toIntArray(re);
	}
	
	/**
	 * Gets the reactions that are not catalyzed in the remaining network as source for the
	 * first search.
	 * 
	...
	private int[] getUncatalyzedReactions() {
		...
		for (int i=0; i<numCatalysts.length; i++)
			if (numCatalysts[i]==0 && !removedReactions.get(i)) re.add(i);
		return NumberTools.toIntArray(re);
	}
	...
	private void createAdjacencListsAsCata() {
		...
		for (int i=0; i<adjListAsCata.length; i++) adjListAsCata[i] = new LinkedList<Integer>();
		for (int i=0; i<network.getNumReactions(); i++)
	...
	private class AutocatalyticSubsetDetectionAction implements NetworkSearchAction {
		...
		/**
		 * Go from a species to a reaction in two case:
		 * Either if the species is a reactant of the reaction -> the reaction cannot be any more
	...
	private class ConnectedComponentIdentificationAction implements NetworkSearchAction {
		...
		/**
		 * Walk from a species to a reaction if the species is reactant and all other reactants have been visited.
		 */
		public boolean checkReaction(int reaction, NeighborType neighborType) {
