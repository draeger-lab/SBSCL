 ...
 * @since 0.9
 */
public class SBMLinterpreter implements SBMLValueHolder {
...
//      initEvents();
    ...
    if (copiedAST == null) {
      ...
      for (ASTNode child : node.getChildren()) {
        if (function!=null) {
          ...
          copiedAST.addChild(copyAST(child, mergingPossible, null, inFunctionNodes));
        }
      }
  ...
  public void updateSpeciesConcentration(AmountManager amountManager){
    ...
    for (int i = 1; i< Y.length; i++){
      Y[i] = amountManager.getAmount(i - 1);
    }
