 ...
 * ---------------------------------------------------------------------
 */
package fern.network.sbml.astnode;

import fern.network.sbml.MathTree;
import fern.network.sbml.SBMLValueHolder;
...
public class ASTNodeInterpreter {
   ...
   * A logger.
   */
  public static final Logger logger = Logger.getLogger(MathTree.class.getName());
  ...
  public double compileDouble(CallableSBase nsb, double time, double delay) {
    if (nsb instanceof Species) {
      ...
      if (r.isSetKineticLaw()) {
        ((ASTNodeValue) r.getKineticLaw().getMath().getUserObject(MathTree.TEMP_VALUE))
  ...
  public boolean compileBoolean(CallableSBase nsb, double time) {
    if (nsb instanceof FunctionDefinition) {
      ...
      ASTNodeValue rightChild = (ASTNodeValue) math.getRightChild().getUserObject(MathTree.TEMP_VALUE);
