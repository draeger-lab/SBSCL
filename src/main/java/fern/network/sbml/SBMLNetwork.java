 ...
 * <code>registerEvents</code>, since the event handling is treated by {@link
 * SBMLEventHandlerObserver}s which need to be attached to the {@link Simulator}.
 *
...
public class SBMLNetwork extends AbstractNetworkImpl {
  ...
  /**
   * Creates a network from a sbmlfile. If the file contains features not supported by FERN, an
   ...
   * @throws FeatureNotSupportedException
   */
  public SBMLNetwork(File file, boolean ignoreExceptions)
      throws FeatureNotSupportedException, IOException, XMLStreamException, ModelOverdeterminedException {
    super(file.toString());
    ...
    if (!ignoreExceptions) {
      ...
      if (document.getModel().getNumFunctionDefinitions() > 0) {
        throw new FeatureNotSupportedException(
            "Function definitions are not allowed at the moment!");
      }
      if (document.getModel().getNumInitialAssignments() > 0) {
        throw new FeatureNotSupportedException(
            "Initial assignments are not allowed at the moment!");
      }
      for (int s = 0; s < document.getModel().getNumSpecies(); s++) {
        if (!document.getModel().getSpecies(s).isSetHasOnlySubstanceUnits()) {
          throw new FeatureNotSupportedException(
              "For each species the hasOnlySubstanceUnits flag has to be set!");
        }
      }
    }
  ...
  /**
   * Registers the {@link SBMLEventHandlerObserver} for each event in the sbml file to the
   * Simulator.
   *
  ...
  /**
   * Saves the current <code>SBMLNetwork</code> to a sbml file with given level and version.
   *
   * @param file    the file to save the network in
