...
/**
 * Base class for modified networks, which implements the full {@link Network} interface. Extending
 * classes should override each method which is different in the modified network (e.g. in the
 * {@link ReversibleNetwork} only reactions are virtually doubled, so each method concerning the
 * species do not change).
 * <p>
 * Note: An extending class should definitely not copy the whole network but should rather redirect
 * indices to the original network when possible in order to minimize the necessary memory.
 *
...
public abstract class ModifierNetwork implements Network {
  ...
  /**
   * Gets the original network. If there is a higher hierarchy of modified networks (e.g. a
   * <code>ExtractSubNetwork</code> of a <code>ReversibleNetwork</code> of a
   * <code>AutocatalyticNetworkExample</code>), the highest network (here the
   * <code>AutocatalyticNetworkExample</code>
   * is returned.
   *
  ...
  /**
   * Gets the parent network. If there is a higher hierarchy of modified networks (e.g. a
   * <code>ExtractSubNetwork</code> of a <code>ReversibleNetwork</code> of a
   * <code>AutocatalyticNetworkExample</code>), the parental network (here the
   * <code>ReversibleNetwork</code>
   * is returned.
   *
