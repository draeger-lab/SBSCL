 ...
 * to Gillespies Direct Method as in {@link GillespieEnhanced}, the fast ones are handled by 
...
public class HybridMaximalTimeStep extends Simulator {
	...
	public void performStep(SimulationController control) {
		...
		changed = false;
		while (delta<Double.POSITIVE_INFINITY && t<=getNextThetaEvent() && t+delta>getNextThetaEvent() && !changed)
			thetaEvent();
	...
	private boolean condition1(int mu) {
		...
		for (int i=0; i<r.length; i++)
			if (getAmount(r[i])<n) return false;
		return true;
	}
