...
public abstract class AbstractTauLeapingPropensityBoundSimulator extends AbstractBaseTauLeaping {
	...
	protected double chooseTauNonCriticals(BitVector criticals) {
		...
		for (int j=0; j<getNet().getNumReactions(); j++) {
			...
			if (verbose) {
				...
				System.out.println("mu"+mu[j]+"\tsigma "+sigma[j]+"\tmu term="+top/Math.abs(mu[j])+"\tsigma term="+top*top/sigma[j]);
				System.out.println(Arrays.toString(f[j]));
				System.out.println();
				if (tau==top/Math.abs(mu[j]) || tau==top*top/sigma[j])
					min = getName()+" "+NetworkTools.getReactionNameWithAmounts(getNet(), j)+"\tmu "+mu[j]+"\tsigma "+sigma[j]+"\tmu term="+(top/Math.abs(mu[j])+"\tsigma term="+top*top/sigma[j]);
			}
	...
	private void preprocessNonCriticals(BitVector criticals) {
		...
		PartialDerivativePropensityCalculator propCalc = (PartialDerivativePropensityCalculator) getNet().getPropensityCalculator();
		
		
		for (int j=0; j<getNet().getNumReactions(); j++) {
			for (int js=0; js<getNet().getNumReactions(); js++) {
				if (criticals.get(js)) continue;
				f[j][js] = 0;
