	...
	protected double chooseTauNonCriticals(BitVector criticals) {
		...
		for (int i=0; i<getNet().getNumSpecies(); i++) {
			...
			if (gi==-2)
				...
				gi=3.0+1.0/(getAmountManager().getAmount(i)-1.0)+2.0/(getAmountManager().getAmount(i)-2.0);
			else if (gi==-4)
			...
			if (verbose) {
				...
				System.out.println(" mu "+mu[i]+"\tsigma "+sigma[i]+"\tmu term="+(max/Math.abs(mu[i])+"\tsigma term="+max*max/sigma[i]));
			}
		}
		return tau;
	}
