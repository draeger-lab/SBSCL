...
/**
 * Observes the calls to the random number generator and count the number of drawn
 * random number of different distributions. These calls are crucial for the stochastic
 * algorithms because besides algebraic calculations are these the most time 
 * consuming operations.
 * <p>
 * This observer does take repeats into account. If you repeat the simulation, 
 * you will get an average over the results each run.
 * 
...
public class RandomNumberGeneratorCallObserver extends Observer implements GnuPlotObserver {
	...
	public void finished() {
		if (numCalls==null) numCalls = NumberTools.convertIntToDouble(Stochastics.getInstance().getCounts());
		else {
			int[] act = Stochastics.getInstance().getCounts();
			for (int i=0; i<numCalls.length; i++) 
				numCalls[i] = (numCalls[i]*(double)getNumSimulations()+act[i])/((double)getNumSimulations()+1.0);
		}
	...
	public GnuPlot toGnuplot(GnuPlot gnuplot) {
		...
		gnuplot.addData(new Object[] {NumberTools.getNumbersTo(numCalls.length-1),numCalls}, true, applyLabelFormat(new String[1]), getStyles());
		return gnuplot;
	}
