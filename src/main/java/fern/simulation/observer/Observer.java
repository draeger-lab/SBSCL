...
/**
 * Implementing classes are used to observe certain aspects of simulations. It is basically just an
 * interface of methods that get called during a simulation together with some common methods useful
 * for many types of observers.
 * <p>
 * Some observers take repeats into account. If you repeat the simulations, they will yield average
 * data.
 * <p>
 * Each observer should override the <code>toString</code> method which should return a string
 * representation of its data.
 * <p>
 * You can register a {@link PrintWriter} to the <code>Observer</code>. If you have, the return
 * value of <code>toString</code> is printed if a simulation has finished.
 *
...
public abstract class Observer {
  ...
  /**
   * Gets called by simulators when a certain moment in time is reached. This moment in time has to
   * be registered by {@link Observer#getTheta()}
   *
   ...
   * @param tau      the time the reaction fires (at this time {@link Simulator#getTime()} does not
   *                 necessarily yield the firing time)
  ...
  /**
   * Gets called after the <code>finish</code> call and prints the results of this observer if a
   * {@link PrintWriter} is registered.
   */
  public void print() {
    if (pw != null) {
      pw.append(toString());
  ...
  /**
   * Applies the actual label format to the whole array (but does not change the passed array, it
   * return a new one)
   *
  ...
  /**
   * Gets a gnuplot command if you want to plot a histogram and have the data labels for each value
   * in an array
   *
  ...
  /**
   * Sets the simulator. For internal use only. If you want to override it, be careful with the
   * consistency between the simulator's observer list and the observer's simulator!
   *
  ...
  /**
   * Gets the number of finished repeats of the simulation. The number is increased after the
   * <code>finished</code> call, so within <code>finish</code> of an extending class it is the
   * number of repeats without the actual simulation.
   *
