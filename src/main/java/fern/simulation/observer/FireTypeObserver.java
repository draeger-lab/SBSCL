...
public class FireTypeObserver extends Observer implements GnuPlotObserver {
	...
	public void finished() {
		for (int i=0; i<numTypes.length; i++)
			numTypesAvg[i] = (numTypesAvg[i]*getNumSimulations()+numTypes[i])/(double)(getNumSimulations()+1);
	}
	...
	public String toString() {
		double sum = 0;
		for (int i=0; i<numTypesAvg.length; i++) sum+=numTypesAvg[i];

		StringBuilder sb = new StringBuilder();
		sb.append(getSimulator().getName());
		sb.append(":\n");
		for (int i=0; i<numTypesAvg.length; i++) 
			sb.append(String.format("%s %d (%.4f) ", FireType.values()[i].toString(),(long)numTypesAvg[i],numTypesAvg[i]/sum));
		return sb.toString();
	}
	...
	public GnuPlot toGnuplot(GnuPlot gnuplot) {
		...
		gnuplot.addData(new Object[] {NumberTools.getNumbersTo(numTypesAvg.length-1),numTypesAvg}, true, applyLabelFormat(new String[1]), getStyles());
		return gnuplot;
	}
