 ...
 * override <code>initialize</code> but should in that case call <code>super.initialize()</code> to
 * avoid unwished effects.
 * <p>
 * Simulators also should handle the special time <code>theta</code>: is is a moment in time when
 * <code>thetaEvent</code> is supposed to be invoked (e.g. to measure species populations at this
 * moment). Consider this, if you want to implement a simulator.
 * <p>
 * The <code>fireReaction</code> are supposed to be invoked when a simulator causes a reaction to
 * fire.
 * <p>
 * If an extending class sticks to these conventions, it can take full benefit of the observer
 * system: One or more {@link Observer} can be registered at a simulator and observe certain aspects
 * of the simulation (see the {@link Observer}s javadoc for more information).
 *
...
public abstract class Simulator {
   ...
   * {@link Simulator#start(SimulationController)}.
   *
  ...
  /**
   * Start the simulation with the given {@link SimulationController}. Basically calls {@link
   * #preRun()},{@link #run(SimulationController)} and {@link #postRun()}.
   *
  ...
  /**
   * Runs a simulation, after it's initialized by calling {@link #preRun()} (allows to continue a
   * simulation).
   *
   * @param control
   */
  ...
  /**
   * Gets called when the simulator reaches the special time <code>theta</code>. It invokes {@link
   * Observer#theta(double)} of the appropriate observer.
   * <p>
...
//				observer[o].theta(theta);
  ...
  /**
   * Initializes the propensities. Also, reinitialize them whenever any event is executed.
   */
  ...
  /**
   * Performs one simulation step. Between steps the terminating condition is checked. The
   * simulators controller is passed, if an extending class wants to check it within one step.
   * Implementing algorithms cannot be sure that the propensities are correct at the beginning of a
   * step (e.g. if the volume changed). They should override {@link Simulator#setAmount(int, long)}
   * and {@link Simulator#setVolume(double)} if they need correct values!
   *
  ...
  /**
   * Theta defines a moment, where the simulator has to invoke <code>theta</code> of a observer. It
   * is used e.g. to determine the amounts of species at one moments. Extending class just have to
   * call {@link Simulator#thetaEvent()} which basically calls the observer.
   *
   * @return the theta
   */
...
//		return theta;
  ...
  private static class DelayedThetaInvocationParameters {
    ...
    public DelayedThetaInvocationParameters(double beforeTheta, long[] beforeThetaAmounts,
        double interpolationTheta, LinkedList<Observer> observers) {
