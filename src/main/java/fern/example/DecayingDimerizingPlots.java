...
public class DecayingDimerizingPlots {
  ...
  public static void main(String[] args) throws IOException, JDOMException {
    ...
    gp.addData(getHistoAsParallelArray(ssaHisto), new String[]{"exact SSA"},
        new String[]{"with linespoints"});
    gp.addData(getHistoAsParallelArray(tlaHisto), new String[]{"Absolute Bound 0.03"},
        new String[]{"with linespoints"});
    gp.addData(getHistoAsParallelArray(tlrHisto), new String[]{"Relative Bound 0.03"},
        new String[]{"with linespoints"});
    gp.addData(getHistoAsParallelArray(tlsHisto), new String[]{"Species Bound 0.03"},
        new String[]{"with linespoints"});
    gp.setVisible(true);
    gp.plot();
    ...
    System.out
        .println("Absolute Bound: " + NumberTools.calculateHistogramDistance(ssaHisto, tlaHisto));
    System.out
        .println("Relative Bound: " + NumberTools.calculateHistogramDistance(ssaHisto, tlrHisto));
    System.out
        .println("Species Bound:  " + NumberTools.calculateHistogramDistance(ssaHisto, tlsHisto));
    System.out.println("(each eps=0.03)");
  }

  private static double[][] getHistoAsParallelArray(Map<Integer, Integer> histo)
      throws IOException {
