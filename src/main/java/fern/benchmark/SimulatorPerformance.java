...
public abstract class SimulatorPerformance extends Benchmark {
	...
	/**
	 * Contains the names of the simulators for using in the <code>present</code>-method of extending classes.
	 */
	protected String[] simulatorNames;
	/**
	 * Contains the number of iterations done for using in the <code>present</code>-method of extending classes.
	 */
	 ...
	 * compromise time benchmarks like caching, the order of the simulators is randomized
	 * each time. The time needed for each algorithm is recorded and added to <code>Benchmark</code>'s
	 * data pool. After each {@link SimulatorPerformance#getShowSteps()} iterations, <code>present</code>
	 * is called.
	 * 
	...
	/**
	 * Is called after <code>getShowSteps</code> iterations. Implement your benchmark
	 * presentation here.
	 */
