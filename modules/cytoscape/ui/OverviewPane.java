...
public class OverviewPane extends JPanel {
  ...
  private void initializeComponents() {
    ...
    c.gridy++;
		if (visu == null) {
			visu = new JCheckBox("visualize", true);
		}
    add(visu, c);

    c.gridx++;
    add(getVisOptionsButton(), c);
  ...
  private Component getVisOptionsButton() {
    return new JButton(new AbstractAction("Colors") {
      private static final long serialVersionUID = 1L;

      public void actionPerformed(ActionEvent arg0) {
        new ColorPicker(main).setVisible(true);
      }
    });
  }
