      ...
      public void windowClosing(WindowEvent arg0) {
				if (progress.getValue() > 0) {
					if (JOptionPane.YES_OPTION == JOptionPane
							.showConfirmDialog(Cytoscape.getDesktop(), "Reset colors to normal?", "FERN",
...
//		defaultNodeAppereanceCalculator = Cytoscape.getVisualMappingManager().getVisualStyle().getNodeAppearanceCalculator();
  ...
  public void reloadNetworkByButton() {
    String[] attr = extended.getAttributeKeys();
		...
		if (attr[0] != null && attr[1] != null && attr[2] != null && attr[3] != null
				&& attr[4] != null) {
        ...
        new Thread(new Runnable() {
          ...
          public void run() {
            try {
              ...
              sim.addObserver(
                  new CytoscapeColorChangeObserver(overview.isVisualize(), overview.isRealTime(),
                      sim, getNetwork(), style, extended.getTrendSpecies(), overview.getTime()));
  ...
  public void loadNetwork(Network net) {
      ...
      nodeAttr.setAttribute(net.getSpeciesName(i), "initial amount", (int) net.getInitialAmount(i));
		...
		if (!(net.getPropensityCalculator() instanceof AbstractKineticConstantPropensityCalculator)) {
			throw new RuntimeException(
					"Up to now only KineticConstantPropensityCalculators are allowed!");
		}

    AbstractKineticConstantPropensityCalculator prop = (AbstractKineticConstantPropensityCalculator) net
    ...
    reloadNetwork(new NetworkChecker("node type", "reaction", "species", "reaction coefficient",
        "initial amount"));
