...
public class NetworkChecker {
  ...
  public NetworkChecker(String nodeType, Object nodeTypeReaction, Object nodeTypeSpecies,
      String coefficient, String initialAmount) {
  ...
  public void check() {
    ...
    // create nodeClassifier
		if (nodeTypeReaction instanceof String) {
			nodeClassifier = new NodeClassifierByAnnotation<String>(nodeType, (String) nodeTypeReaction,
					(String) nodeTypeSpecies);
		} else if (nodeTypeReaction instanceof Integer) {
			nodeClassifier = new NodeClassifierByAnnotation<Integer>(nodeType, (Integer) nodeTypeReaction,
					(Integer) nodeTypeSpecies);
		} else if (nodeTypeReaction instanceof Double) {
			nodeClassifier = new NodeClassifierByAnnotation<Double>(nodeType, (Double) nodeTypeReaction,
					(Double) nodeTypeSpecies);
		} else {
		...
		if (!nodeClassifier.isUsable()) {
			throw new IllegalArgumentException(
					"Could not distinguish between reactions and species!\nTry to choose valid keys for the nodes.");
		}
    ...
    nodeParameter = new NodeParameter(coefficient, initialAmount);
		...
		if (!nodeParameter.isUsable()) {
			throw new IllegalArgumentException(
					"The nodes do not contain reaction coefficients / initial amounts!");
		}
    ...
    public double getReactionCoefficient(Node n) {
      ...
      switch (nodeAttr.getType(coeff)) {
        ...
        case CyAttributes.TYPE_INTEGER:
          ...
          break;
        default:
          throw new RuntimeException(coeff
              + " has wrong type or is not present as identifier for the reaction coefficients in reaction nodes.\nOnly Double and Integer are permitted!\n\nTry to load node attributes or change the field name!");
      }
			if (re == null) {
				throw new RuntimeException(coeff
						+ " has wrong type or is not present as identifier for the reaction coefficients in reaction nodes.\nOnly Double and Integer are permitted!\n\nTry to load node attributes or change the field name!");
    ...
    public long getSpeciesInitialAmount(Node n) {
      ...
      switch (nodeAttr.getType(initAm)) {
        ...
        case CyAttributes.TYPE_INTEGER:
          ...
          break;
        default:
          throw new RuntimeException(initAm
              + " has wrong type or is not present as identifier for the initial amounts in species nodes.\nOnly Double and Integer are permitted!\n\nTry to load node attributes or change the field name!");
      }
			if (re == null) {
				throw new RuntimeException(initAm
						+ " has wrong type or is not present as identifier for the initial amounts in species nodes.\nOnly Double and Integer are permitted!\n\nTry to load node attributes or change the field name!");
      ...
      switch (nodeAttr.getType(initAm)) {
        ...
        default:
          throw new RuntimeException(
              initAm + " has wrong type. Only Double and Integer are permitted!");
      }
    }
  ...
  public class NodeClassifierByAnnotation<T> implements NodeClassifier {
    ...
    public boolean isReactionNode(Node n) {
			if (reactionType instanceof String) {
				...
				throw new RuntimeException(
						"Only String, Integer and Double allowed as node identifier type!");
			}
    }
  ...
  public class EdgeClassifierByDirection implements EdgeClassifier {
    ...
    public boolean isReactionToProductEdge(Edge e) {
      return nodeClassifier.isReactionNode(e.getSource()) && nodeClassifier
          .isSpeciesNode(e.getTarget());
    }

    public boolean isReactionToReactantEdge(Edge e) {
      return nodeClassifier.isSpeciesNode(e.getSource()) && nodeClassifier
          .isReactionNode(e.getTarget());
    }
      ...
      for (Iterator<NodeView> i = view.getNodeViewsIterator(); i.hasNext(); ) {
        ...
        int index = network.getIndex(node);
				if (nodeClassifier.isReactionNode(node)) {
					if (network.getAdjacentEdgeIndicesArray(index, false, true, false).length <= 0
							|| network.getAdjacentEdgeIndicesArray(index, false, false, true).length <= 0) {
						return false;
					}
				}
      }
