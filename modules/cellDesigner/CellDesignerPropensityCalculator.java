  ...
  public CellDesignerPropensityCalculator(PluginModel model, CellDesignerNetworkWrapper net) {
    ...
    for (int i = 0; i < model.getNumReactions(); i++) {
      ...
      for (int j = 0; j < reaction.getKineticLaw().getNumParameters(); j++) {
        localParameter.put(reaction.getKineticLaw().getParameter(j).getId(),
            reaction.getKineticLaw().getParameter(j).getValue());
      }
      ASTNode a = reaction.getKineticLaw().getMath();
      propensities[i] = new MathTree(net, reaction.getKineticLaw().getMath(), globalParameter,
          localParameter, net.getSpeciesMapping());
    }
