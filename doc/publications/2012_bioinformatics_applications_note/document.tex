...
model’s components. It is based on the Java\texttrademark{} library JSBML
\citep{Draeger2011b}, a specifically developed data structure to read and write models from and into
...
simulation core has also been integrated into the widely used program
CellDesigner version 4.2 \citep{Funahashi2003} as an internal simulation
library.
...
solver is slower than the other solvers, but it can be used for solving stiff
...
CellML, \citealt{Lloyd2004}) can be added by only implementing a interpreter for
each format.
