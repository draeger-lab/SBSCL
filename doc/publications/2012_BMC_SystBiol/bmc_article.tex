...
such as \COR{BioModels Database} \cite{Li2010a} \COR{and} the CellML model
...
now support the \acs{SBML} data format (April 18\textsuperscript{th} 2013).}} in
...
a platform-independent, well-tested generic opensource library.
...
\COR{When}\sout{In case that} the system under study operates at multiple time
...
\COR{function definitions} are integrated into \sout{one}\COR{a single} directed
acyclic syntax graph.
This graph is \sout{hence}\COR{thus}
...
After the creation of th\sout{e abstract syntax}\COR{is} graph, the initial
assignments and the assignment rules (including transformed algebraic rules) are
processed.
\COR{Initial values defined by initial assignments or assignment rules are
...
In Level~3 Version~1 \sout{just}\COR{only} a few new language elements have been
...
\sout{For}\COR{At} every time step, the events to be executed are a union of two subsets of the set of all events.
On \sout{the} one hand, \COR{there are events whose triggers have} been activated at the \COR{current} time and which are to be evaluated without delay.
...
For every element of the resulting set of events\sout{ their}\COR{, the} priority rule must be evaluated.
One event is randomly chosen for execution from all events \COR{of} highest priority.
\COR{In principle, a}ll other events could be handled in the same manner\COR{, but}
...
In this case, \sout{another}\COR{the} event that has now the highest priority is chosen\COR{ as next}.
...
If the error tolerance cannot be \sout{ensured}\COR{respected},
...
Note that function $\vec{g}$ returns 0 in all \COR{dimensions,
...
\sout{library}\COR{community project}, which has been designed 
...
versions.
This can be explained by the fact that \sout{there are}\COR{the test suite
contains} some models
\sout{in \acs{SBML} Level~3 Version~1}\COR{of this version}, 
...
\href{http://biomodels.net}{BioModels Database}, a resource that contains a
...
all 424 curated models from \COR{BioModels Database}
(release~23, October 2012) \COR{without raising any errors, see Methods
for details}.
...
However, this precise timing can lead to a noticeable increase in run-time when events 
are triggered in very small intervals, e.g., every $0.001$ time units.
This behavior can, for example, be observed in BioModels Database model \numero~408.
When the precise timing of events is not of utmost importance, a different
solver other than the Rosenbrock solver can be chosen. But there are already ambitions
to improve the runtime behavior during the precise timing of events.}
\sout{The Rosenbrock solver with the adaption of its step size is capable of
solving stiff \acsp{ODE} and, according to our experiments, performs better than
other solvers in this library.
Albeit there are some models from BioModels Database that constitute very stiff
problems or large \acsp{ODE} (e.g., the models \numero~235 or \numero~408),
with which Rosenbrock's method has some difficulty solving them.
In order to maintain the desired accuracy, the solver continuously decreases its
step size.
As a result, the time consumption for simulation rises.
In some cases, tuning the relative and absolute tolerances can help, but
depending on the system's structure, Rosenbrock's method is sometimes stretched
to its limits.}

\sout{The Runge-Kutta-Fehlberg method \cite{Fehlberg1970} (\acs{KiSAO} term 86),
which is included in iBioSim, also shows an increase in run time with the models
mentioned above.}
\COR{When dealing with stiff problems, Rosenbrock's method is a good choice,
since it is intended for stiff \acsp{ODE}.
...
Whereas more advanced solvers, such as \textsc{CVODE} from \textsc{SUNDIALS}
\cite{Hindmarsh2005}, can adapt for non-stiff and stiff problems, the Rosenbrock
solver cannot.
This issue can lead to an increase in run-time regarding large models such as
model \numero~235 of the BioModels Database. In some cases, tuning the relative
and absolute tolerance can help, but depending on the system's structure,
Rosenbrock's method is sometimes stretched to its limits. 
...
is included in iBioSim, shows also an increase in run time concerning the model
mentioned above.}
\TODO{It should be mentioned that iBioSim does not primary focus on the use of
\acs{ODE} solvers, but in the stochastic analysis of systems with the Reb2Sac
stochastic simulator and abstraction techniques to make the models easier to
analyze \cite{Madsen2012}.}
...
\textsc{CVODE}\sout{  from \textsc{SUNDIALS} \cite{Hindmarsh2005}}, which is 
incorporated into BioUML.
\textsc{SUNDIALS} can handle complicated \acsp{ODE} significantly better, but
since it is not available under the \acs{LGPL} and no open-source Java version
of these solvers can currently be obtained, we disregard its use.}

\TODO{Nevertheless, the Rosenbrock solver embodies a universal approach for
...
While the unbound variable of each such equation can be efficiently identified,
the transformation of an algebraic rule into an assignment rule includes
symbolic computation and is very difficult to implement.
...
could be identified using nested intervals.
...
However, the algorithm discussed in this paper describes the core features of
\acs{SBML}.
The features added to core \acs{SBML} by these packages are very diverse,
reaching the graphical representation \cite{Gauges2006}, the description
of qualitative networks, such as Petri nets \cite{Chaouiya2012}, and many more. 
It is therefore necessary to separately derive and implement algorithms for the
interpretation of individual extension packages.}
...
\TODO{This only describes the implementation, but does not say much about the
algorithmic part of the paper!}
...
\sout{the Systems Biology Simulation Core Library}\COR{iBioSim}.
...
%
\sout{Therefore, t}\COR{T}he Systems Biology Simulation Core
...
\sout{additional}\COR{further} model formats, such as CellML,
\sout{besides}\COR{in addition to} its \acs{SBML} implementation.
...
determination of the exact time at which an event occurs \sout{as}\COR{because}
...
In the \SBMLinterpreter\COR{,} events are represented via an array containing
\sout{an object}\COR{one instance} of \sout{the class} \EventInProgress for
...
\TODO{An event with delay can produce multiple further assignments within
the time frame since it has been triggered for the first time and the actual
execution of its first assignment.}
In order to deal with \sout{such an issue}\COR{delayed events}, the class
...
they have to be \sout{ordered according to}\COR{sorted in ascending order by}
...
For this reason, we disregarded the use of other data structures for the current
implementation.}
...
%
\COR{Since \acs{SBML} Level~2 Version~1 it has als become possible to create
...
several steps.
The arguments of the respective function are evaluated and then passed to their
corresponding node in the graph via the mapping established before.
...
\COR{The constraints introduce assumptions about a model behavior.
...
\COR{All curated models from BioModels Database have been simulated with
identical settings, as it was suggested before \cite{Bergmann2008}:
...
For the models \numero~234 and \numero~339 from BioModels Database the absolute tolerance had to be set to $10^{-10}$ in order to achieve the necessary accuracy
and to avoid that the algorithm surpasses its minimal step size.}

\subsection*{Simulation of the SBML Test Suite}

\COR{The models from SBML Test Suite version~2.3.2 were first simulated with the Rosenbrock solver, $10^{-6}$ as relative and $10^{-12}$ as absolute tolerance. 
...
This means that a \COR{\emph{nonpersistent}} event can be removed from $E_\mathrm{A}$ \sout{in case that}\COR{when} its trigger condition becomes \false during the evaluation of other events.
\sout{Here, t}\COR{T}he function $\mathrm{trig}(e)$ returns \COR{1 or 0} depending on whether or not the trigger condition of event $e \in E$ is satisfied.
Similarly, the function $\mathrm{persist}(e)$ returns \COR{1} if event $e$ is \sout{not} persistent, or \COR{0} otherwise.
\COR{At} each iteration\COR{,} the trigger conditions of \sout{those} active events $e_\mathrm{a} \in E_\mathrm{A}$ that are not persistent are checked.
If the trigger condition of such an event has changed from \true (1) to \false (0), \sout{it}\COR{the event} is removed from $E_\mathrm{A}$.
...
\subsection*{Figure 4 - \sout{Integrated calculation of new values for a time step including event processing}\COR{Refined step-size adaptation for events}}
...
bisphosphate\COR{, a key metabolite for \ce{CO2} fixation in the reaction catalyzed by \acf{RuBisCO},} and the currency metabolites \acs{ATP} and \acs{ADP} during the first 35\,s of
the photosynthesis.
...
The table gives an overview about the most characteristic features of \acs{SBML}-capable simulation programs \COR{(April 18\textsuperscript{th} 2013)}.
\sout{As one aspect, i}\COR{I}t shows which programs support \acs{SBML} elements fast reactions, algebraic rules, and events.
...
\sout{Please n}\COR{N}ote that in the \COR{SBML Test Suite column, a dash means that not all of its models can be correctly solved, because not all SBML elements are supported.} 
...
\COR{The measured running time consists of the times necessary for the interpretation and for the simulation of the models. The reading of the SBML model into memory is not considered at this point.}
An Intel\textsuperscript{\textregistered} Core\texttrademark{} i5 CPU with 3.33\,GHz  and 4\,GB RAM was used with
Windows\textsuperscript{\textregistered}~7 (Version 6.1.7600) as operating system \COR{and Java Virtual Machine version 1.6}. 
\par \mbox{}
\par
    \mbox{
