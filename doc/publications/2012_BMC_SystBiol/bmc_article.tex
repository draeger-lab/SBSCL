...
\COR{BioModels Database} \cite{Novere2006a, BioModels2010} \COR{and} the CellML model repository
...
\COR{to facillitate the execution of \acs{SED-ML} files}.
...
However, in case that such an event is cancelled by other events, also all of its assignments are canceled before execution.
...
For biomodel \numero~234 the absolute tolerance had to be set to $10^{-10}$ in order to achieve the necessary accuracy.}

\subsection*{Simulation of the SBML Test Suite}

Test Suite version~2.3.2.
...
Generally, these valuves can be variable.
...
In Level~3 the \StoichiometryMath element has been removed from the langugage definition.
...
constructs.
Hence, iBioSim, the Systems Biology Simulation Core Library, and BioUML are the 
