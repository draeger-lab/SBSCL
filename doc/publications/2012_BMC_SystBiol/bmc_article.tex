...
\COR{The modifications done to the Rosenbrock solver enables a precise timing of
events during simulation. 
Albeit when events are triggered in small intervals, this can lead to noticeable
increase in run-time.
This can, for example, be observed in BioModels Database model \numero~408.
When the precise timing of events is not of utmost importance, a different
solver other than the Rosenbrock solver can be chosen. But there are ambitions
...
However, the experiments show, that the Rosenbrock solver can be inefficient for
...
\COR{\emph{nonpersistent}} event can be removed from $E_\mathrm{A}$ \sout{in case that}\COR{when} its trigger condition becomes \false during the evaluation of other events. Here, the function $\mathrm{trig}(e)$ returns \COR{1 or 0} depending on whether or not the trigger condition of event $e \in E$ is satisfied. Similarly, the function
